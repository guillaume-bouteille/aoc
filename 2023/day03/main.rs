...
fn parse_inputs() -> Game {
    ...
    for (y, line_it) in io::stdin().lines().enumerate() {
        if let Ok(line) = line_it {
            let mut line_obj = {Line{y:y as i32, numbers:Vec::new(), engines:Vec::new()}};
...
//            println!("Number {} at {},{}", num.value, num.start_x, line_num.y);
            // TODO: pas compris pourquoi l'objet sur lequel on iter en retour de get, c'est pas
            // directement un T, mais un iterateur de T...
            for truc in game.lines.get(min_y..max_y+1) {
                for line_engine in truc {
...
//                            println!("Engine {} at {},{} adjacent to number {} at {},{}", engine.symbol, engine.x, line_engine.y, num.value, num.start_x, line_num.y);
    ...
    println!("Result P1 is {}", sum_adjacent);
    ...
    println!("Result P2 is {}", sum_gear_ratio);
}
